...
const newStrapLength = (strapArray) => {
  ...
  strapArray.forEach((listElement) => {
    ...
    //Add event listeners for updating left and right strap lengths, based on form inputs
    strapLengthForm.addEventListener("submit", (e) => {
      //Stop the submit button from attempting to reload the page
      e.preventDefault();

      //Make the submit action do something
      //Get the value from the form input element
      let newLength = strapLengthForm.querySelector("input").value;

      //Set the value based on form input
      listElement.querySelector("span").innerHTML = `${newLength} inches`;

      //Clear the value from the form once the change is submitted
...
const backpackList = backpackObjectArray.map((backpack) => {
  ...
  `;
  //Get the length of both straps of the backpack
  let strapLengths = backpackArticle.querySelectorAll(".backpack__strap");

  //Call newStrapLength function, passing in the node list (array) 'strapLengths'
  newStrapLength(strapLengths);

  //Identify the button to receive the event listener to toggle the state of the backpack
